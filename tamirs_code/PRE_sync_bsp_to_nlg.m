function [p,bsp_data]=PRE_sync_bsp_to_nlg(p,bsp_data)
...
for ii_tag = 1:length(files_to_convert)
    clear bsp_pos
    %load( fullfile(bsp_dir, files_to_convert(ii_file).name) );
    bsp_pos.ts_nlg_usec = interp1(sync_ts.bsp, sync_ts.nlg, bsp_data(ii_tag).ts_ns, 'linear','extrap');
