...
if ~isempty(sync_jump_ts)
    ...
    sync_ts.X = sort([matching_TTL_X new_ttl_X])/unit_x*1e-3;
    sync_ts.Y = sort([matching_TTL_Y new_ttl_Y])/unit_y*1e-3;
else
    sync_ts.X = matching_TTL_X/unit_x*1e-3;
    sync_ts.Y = matching_TTL_Y/unit_y*1e-3;
end
...
plot(edges2centers(1e-3*sync_ts.X), (unit_x.*diff(sync_ts.X)) ./ (unit_y.*diff(sync_ts.Y)),'.-')
plot(repmat(sync_jump_ts,2,1), repmat(get(gca,'ylim'),length(sync_jump_ts),1)', 'm-')
rescale_plot_data('x', [1e-3/60 pairs_good_x_ts(1)]);
