function [bsp_full_data_pos,bsp_full_data_ts] = POS_fill_holes_also_in_y(raw_data_pos,raw_data_ts_usec,bsp_dir)
%shir's code. Modified to fill holes also in y by Ayelet May 2019
...
%remove!
% figure_folder=[bsp_dir,'\holes_fig\'];
...
% change to find velocity in x:
vel = [0 diff(raw_data_pos(1,:))./(diff(raw_data_ts_usec)*10^-6)];
vel_smooth_win = 2/(min_ts_diff*10^-6); %the number of samples in 2 sec
vel = smooth(vel,vel_smooth_win)';
repetitions_ind = find(and(vel>=vel_TH,diff([0 raw_data_pos(1,:)])==0));
...
%     end
    if holes_size_t(hole_i)<=(interpolation_always_TH) %small hole - ALWAYS LINEAR INTERPOLATION
        ...
        fill_in_pos={};
        for dim_i=1:2 % change to work on 2 dimentions
            if dim_i==2
                %for y take only the two smaples close to the hole because
                %it is so noisy
                pos_for_fitting = raw_data_pos(dim_i,[side1_ind(end) side2_ind(1)]);
                ts_for_fitting = raw_data_ts_usec([side1_ind(end) side2_ind(1)]);
                
            else
                ts_for_fitting = raw_data_ts_usec([side1_ind side2_ind]);
                pos_for_fitting = raw_data_pos(dim_i,[side1_ind side2_ind]);
            end
            coeffs = fit(ts_for_fitting',pos_for_fitting','poly1');
            fill_in_pos{dim_i} = polyval([coeffs.p1 coeffs.p2],fill_in_ts);
            
        end
        % change to insert the matrix:
        fill_in_pos_mat=cell2mat(fill_in_pos);
        fill_in_pos_mat=reshape(fill_in_pos_mat,[],2)';
...
%         title(['intrepolation hole #',num2str(hole_i)])
%         legend('fill in','original')
%         file_name=[figure_folder,'hole #',num2str(hole_i),'.jpg'];
...
%         clf
    ...
    pos_side1 = raw_data_pos(1,side1_ind); %change velocity only in x
    coeffs_side1 = fit(ts_side1',pos_side1','poly1');
    vel_side1 = coeffs_side1.p1; %m/usec
    
    ts_side2 = raw_data_ts_usec(side2_ind);
    pos_side2 = raw_data_pos(1,side2_ind);%change velocity only in x
    coeffs_side2 = fit(ts_side2',pos_side2','poly1');
    vel_side2 = coeffs_side2.p1; %m/usec
    
    vel_hole = (raw_data_pos(1,holes_ind_end(hole_i)) - raw_data_pos(1,holes_ind_start(hole_i)))/holes_size_t(hole_i);
    ...
    if max([side1_vs_side2, hole_vs_side1, hole_vs_side2])>TH_velocity_index
        ...
        middle_nans = repmat(fill_in_ts,2,[]); %change to work with y
        middle_nans(:) = nan;
        ...
        if flag_change_hole_i %then keep original raw samples
            ...
            for dim_i=1:2 %change to work with y %DEBUG!!
                middle_nans(dim_i,ind_raw_samples) = raw_data_pos(1,holes_ind_start(hole_i)+1:...
                    holes_ind_start(hole_i)+flag_change_hole_i);
            end
        end
        bsp_full_data_pos = [bsp_full_data_pos middle_nans];
    ...
    if holes_size_t(hole_i) < large_hole_TH  %interpolation
        fill_in_pos={};
        for dim_i=1:2 %changed to work with y
           if dim_i==2
                %for y take only the two smaples close to the hole because
                %it is so noisy
                pos_for_fitting = raw_data_pos(dim_i,[side1_ind(end) side2_ind(1)]);
        ...
        if flag_change_hole_i
            ...
            fill_in_ts(ind_raw_samples) = raw_data_ts_usec(holes_ind_start(hole_i)+1:...
                holes_ind_start(hole_i)+flag_change_hole_i);
            fill_in_pos_mat(:,ind_raw_samples) = raw_data_pos(:,holes_ind_start(hole_i)+1:...
                holes_ind_start(hole_i)+flag_change_hole_i);
        end
...
%         clf
        ...
        for dim_i=1:2 %change to work with y
            ts_side1 = raw_data_ts_usec(side1_ind_extra);
            pos_side1 = raw_data_pos(dim_i,side1_ind_extra);
            coeffs_side1 = fit(ts_side1',pos_side1','poly1');
            
            ts_side2 = raw_data_ts_usec(side2_ind_extra);
            pos_side2 = raw_data_pos(dim_i,side2_ind_extra);
            coeffs_side2 = fit(ts_side2',pos_side2','poly1');
            ...
            fill_in_pos_side1{dim_i} = polyval([coeffs_side1.p1 coeffs_side1.p2],fill_in_ts_side1);
            
            fill_in_ts_side2 = fill_in_ts((end-samples_to_fill_eachside_extrapolation+1):end);
            fill_in_pos_side2{dim_i} = polyval([coeffs_side2.p1 coeffs_side2.p2],fill_in_ts_side2);
            
            middle_nans = zeros([1,total_samples_inHole-(2*samples_to_fill_eachside_extrapolation)]);
            middle_nans(:) = nan;
            middle_nan_dim{dim_i}=middle_nans; %%% change test~
        end
        % change to insert the matrix:
        fill_in_pos_side1_mat=cell2mat(fill_in_pos_side1);
        fill_in_pos_side1_mat=reshape(fill_in_pos_side1_mat,[],2)';
        fill_in_pos_side2_mat=cell2mat(fill_in_pos_side2);
        fill_in_pos_side2_mat=reshape(fill_in_pos_side2_mat,[],2)';
        middle_nan_dim_mat=cell2mat(middle_nan_dim); %%% change test~
        middle_nan_dim_mat=reshape(middle_nan_dim_mat,[],2)';
        
        fill_in_pos_mat = [fill_in_pos_side1_mat middle_nan_dim_mat fill_in_pos_side2_mat];
        if flag_change_hole_i
            [~,ind_raw_samples] = min(abs(raw_data_ts_usec(holes_ind_start(hole_i)+1:...
                holes_ind_start(hole_i)+flag_change_hole_i)' - fill_in_ts),[],2);
            fill_in_ts(ind_raw_samples) = raw_data_ts_usec(holes_ind_start(hole_i)+1:...
...
%         hold on;
%         plot(raw_data_pos(1,[side1_ind_extra side2_ind_extra]),raw_data_pos(2,[side1_ind_extra side2_ind_extra]),'b.')
%         title(['Extrapolation hole #',num2str(hole_i)])
%         legend('fill in','original')
%         file_name=[figure_folder,'hole #',num2str(hole_i),'.jpg'];
%         saveas(gcf,file_name)
%         clf
