function [Samples2,Timestamps2,SampleFrequency]= FIRfilterNlxSlide(filename,t_start_end,passband,slidwin)

% syntax: 
% [Samples,Timestamps]= FIRfilterNlxSlide(filename,tstart,tend,passband,fwin,rwin)
%  
% 
...
if isempty(t_start_end)
    ExtractMode = 1;                                               % We want to extract all the records in the file
else
    ExtractMode = 4;                                               % We want to extract all the records according to given timestamsps
end
ExtractionModeArray = t_start_end;

[ts_blocks,~,~,~,Samples,NlxHeader] = Nlx2MatCSC(filename, FieldSelection,1,ExtractMode,ExtractionModeArray) ;
